...
import api, { CURRENT_USER_ROLE, getApiErrorMessage } from '../services/api';
...
export default function Admin() {
  const [metrics, setMetrics] = useState<Estadisticas>({});
  const [loading, setLoading] = useState(true);
  const [simulating, setSimulating] = useState(false);
  const [error, setError] = useState('');
  const [simulation, setSimulation] = useState<SimulacionMonitoreoResponse>();
  ...
  useEffect(() => {
    ...
    setLoading(true);
    setError('');
    api.get<Estadisticas>('/admin/estadisticas', { headers: { 'X-User-Role': CURRENT_USER_ROLE }, signal: controller.signal })
  ...
  async function simulateMonitoring() {
    ...
    try {
      const response = await api.post<SimulacionMonitoreoResponse>('/notificaciones/simular-inicio');
  ...
  return (
    <div className="space-y-8">
      <header className="flex flex-col justify-between gap-5 sm:flex-row sm:items-end">
        ...
        <Button variant="outline" disabled={loading} onClick={() => setReloadVersion((current) => current + 1)}><RefreshCw className={`size-4 ${loading ? 'animate-spin' : ''}`} />Actualizar</Button>
      </header>
      ...
      <Card className="rounded-2xl border-blue-100 bg-gradient-to-br from-white to-blue-50/60">
        ...
        <CardContent className="flex flex-col items-start justify-between gap-5 px-6 pb-6 sm:flex-row sm:items-end">
          ...
          <Button className="shrink-0" disabled={simulating} onClick={simulateMonitoring}>{simulating ? <LoaderCircle className="size-4 animate-spin" /> : <Activity className="size-4" />}Ejecutar simulación</Button>
        </CardContent>
      </Card>
    </div>
  );
...
function MetricCard({ name, value }: { name: string; value: number }) {
  ...
  return <Card className="rounded-2xl"><CardContent className="p-5"><div className="flex items-start justify-between"><span className="grid size-10 place-items-center rounded-xl bg-blue-50 text-blue-700"><Icon className="size-5" /></span><span className="text-3xl font-black tracking-tight">{value.toLocaleString('es-AR')}</span></div><p className="mt-5 text-sm font-semibold text-slate-700">{formatMetricName(name)}</p></CardContent></Card>;
}
