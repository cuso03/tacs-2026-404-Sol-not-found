...
describe('Métricas de Ciclo de Vida de Actividades', () => {
  ...
  describe('Métrica: Actividad_Reprogramada', () => {
    it('incrementa Actividad_Reprogramada al cancelar votación por falta de quórum', async () => {
    ...
    it('acumula Actividad_Reprogramada en múltiples cancelaciones sucesivas', async () => {
      for (let i = 0; i < 3; i++) {
        const { actividadId, votacionId } = await seedActividadConVotacion({
          min_participantes: 2,
          participantes: [AUTH_ORGANIZADOR],
